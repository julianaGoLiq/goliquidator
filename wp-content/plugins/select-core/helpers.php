...
if(!function_exists('qode_core_portfolio_ajax_load_more')){
	...
	check_ajax_referer( 'qodef_ptf_load_more_nonce_' . sanitize_text_field( $_POST['ptf_load_more_id'] ), 'ptf_load_more_nonce' );

	if (!empty($_POST['type'])) {
        $shortcode_params['type'] = $_POST['type'];
    } else {
		$shortcode_params['type'] = 'gallery';
	}
...
if ( class_exists( 'WP_Block_Type' ) && defined( 'QODE_ROOT' ) ) {
	add_action( 'admin_head', 'suprema_qodef_meta_box_add' );
} else {
	add_action( 'add_meta_boxes', 'suprema_qodef_meta_box_add' );
}


if (!function_exists('ayro_qodef_create_meta_box_handler')) {

	function suprema_qodef_create_meta_box_handler( $box, $key, $screen ) {
		add_meta_box(
			'qodef-meta-box-'.$key,
			$box->title,
