...
try{
	...
	function rev_slider_shortcode($args, $mid_content = null){
		...
		if(empty($slider_alias)){
			if(function_exists('is_user_logged_in') && is_user_logged_in()){
				return '<div><img src="' . RS_PLUGIN_URL_CLEAN . 'admin/assets/images/revlogo.png"></div>';	
			}
			else return '';
		}
	...
	if(is_admin()){
		...
		if(isset($_GET['pk'])){
			$rsssadmin = new RevSliderFunctionsAdmin();
			$sliders = $rsssadmin->get_v5_slider_data();
			echo '<pre>';
			print_r($sliders);
			echo '</pre>';
			exit;
