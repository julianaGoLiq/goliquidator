...
rs-module .material-icons {
  ...
  -moz-osx-font-smoothing: grayscale;
	...
	public function get_html_hide_after_loop(){
		...
		return ($hal !== 0) ? ' data-hideafterloop="'.$hal.'"' : '';
	}
	...
	public function js_get_root(){
		...
		*/
		...
		if($s_type !== 'hero'){
			...
			if($loopSingle == 'noloop' && $this->get_is_single_slide() === true){
				$stopAtSlide = 1;
				$stopAfterLoops = 0;
			}
			...
			/*if($s->get_param(array('general', 'slideshow', 'slideShow'), true) === false){
				$keys['stopAfterLoops']['v'] = 0;
				$keys['stopAtSlide']['v'] = 1;
			}/*else{
				if($stopSlider === true && $stopAfterLoops !== 0 && $stopAtSlide !== 1){
