...
class SupremaQodefLike {
	...
	public function add_like_blog_list($blog_id){
		...
		return '<a class="hover_icon '. $class .'" data-type="portfolio_list" id="qodef-like-'. $blog_id .'-'. $rand_number.'" title="'. $title .'" data-post-id="' . esc_attr( $blog_id ) . '">' . wp_nonce_field( 'qodef_like_nonce_' . esc_attr( $blog_id ), 'qodef_like_nonce_' . esc_attr( $blog_id ), true, false ) .'</a>';
	}
