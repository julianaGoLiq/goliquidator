	...
	function suprema_qodef_pagination($pages = '', $range = 4, $paged = 1){
		...
		if($pages != 1){
			if($show_load_more == 'yes'  && $search_template !== 'yes'){
				$params = array(
					'text' => esc_html__('Load More', 'suprema')
				);
				...
				echo '<ul class="clearfix">';
					...
					for ($i=1; $i <= $pages; $i++){
						if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems )){
							echo suprema_qodef_get_module_part(($paged == $i) ? "<li class='active'><span>" . $i . "</span></li>" : "<li><a href='" . get_pagenum_link($i) . "' class='inactive'>" . $i . "</a></li>");
						}
					}
